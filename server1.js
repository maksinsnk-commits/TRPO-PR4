...
function insertTestData() {
    ...
    const maintenanceData = [
        ...
        [3, `${currentYear}-${currentMonth.toString().padStart(2, '0')}-15`, 'регламентное', 'Регулировка гидравлики', 4500.00, 0.00, 'Сергеев П.К.', 'completed', 5, 'low', 4.0],
        [1, `${currentYear}-${currentMonth.toString().padStart(2, '0')}-25`, 'регламентное', 'Очередное ТО', 4000.00, 800.00, 'Козлов М.С.', 'in_progress', 4, 'medium', null],
        [4, `${currentYear}-${(currentMonth + 1).toString().padStart(2, '0')}-05`, 'внеплановое', 'Ремонт системы охлаждения', 6000.00, 1500.00, 'Сергеев П.К.', 'planned', 6, 'high', null]
    ];
        ...
        return new Promise((resolve, reject) => {
            ...
            maintenanceData.forEach(data => {
                maintenanceStmt.run(data, (err) => {
                    if (err) {
                        console.error('Ошибка добавления обслуживания:', err);
                        reject(err);
                        return;
                    }
                    completed++;
...
app.get('/api/low-stock-parts', (req, res) => {
    const query = `
        SELECT 
            ...
            category,
            CASE 
                WHEN quantity <= min_quantity THEN min_quantity - quantity
                ELSE 0 
...
app.post('/api/clients', (req, res) => {
    ...
    db.run(query, [name, contact_person, phone, email || null, address || null], function(err) {
        if (err) {
            console.error('Ошибка добавления клиента:', err);
            res.status(500).json({ error: err.message });
            return;
...
app.put('/api/clients/:id', (req, res) => {
    ...
    db.run(query, [name, contact_person, phone, email || null, address || null, id], function(err) {
        if (err) {
...
app.post('/api/equipment', (req, res) => {
    ...
    if (!name || !serial_number || !client_id) {
        return res.status(400).json({ error: 'Не заполнены обязательные поля' });
    }
    ...
    db.run(query, [name, model || null, serial_number, location || null, client_id, installation_date || null], function(err) {
        if (err) {
            console.error('Ошибка добавления оборудования:', err);
            res.status(500).json({ error: err.message });
            return;
...
app.put('/api/equipment/:id', (req, res) => {
    ...
    db.run(query, [name, model || null, serial_number, location || null, client_id, installation_date || null, id], function(err) {
        if (err) {
...
app.post('/api/parts', (req, res) => {
    ...
    if (!name || !part_number || quantity === undefined || min_quantity === undefined) {
        return res.status(400).json({ error: 'Не заполнены обязательные поля' });
    }
    ...
    db.run(query, [name, part_number, quantity, min_quantity, price || 0, supplier || null, category || null], function(err) {
        if (err) {
            console.error('Ошибка добавления запчасти:', err);
            res.status(500).json({ error: err.message });
            return;
...
app.put('/api/parts/:id', (req, res) => {
    ...
    db.run(query, [name, part_number, quantity, min_quantity, price || 0, supplier || null, category || null, id], function(err) {
        if (err) {
...
app.post('/api/maintenance', (req, res) => {
    ...
    if (!equipment_id || !maintenance_date || !type || !description || !duration_hours) {
        return res.status(400).json({ error: 'Не заполнены обязательные поля' });
    }
    ...
    db.run(query, [equipment_id, maintenance_date, type, description, work_cost || 0, parts_cost || 0, technician || null, duration_hours, difficulty || 'medium'], function(err) {
        if (err) {
...
app.put('/api/maintenance/:id', (req, res) => {
    ...
    db.run(query, [equipment_id, maintenance_date, type, description, work_cost || 0, parts_cost || 0, technician || null, status || 'planned', duration_hours, difficulty || 'medium', actual_hours || null, id], function(err) {
        if (err) {
...
app.put('/api/maintenance/:id/complete', (req, res) => {
    ...
    db.get("SELECT start_time, duration_hours FROM maintenance WHERE id = ?", [id], (err, row) => {
        ...
        db.run("UPDATE maintenance SET end_time = ?, actual_hours = ?, status = 'completed' WHERE id = ?", 
               [endTime, actualHours, id], function(err) {
            if (err) {
...
app.post('/api/service-requests', (req, res) => {
    ...
    if (!client_name || !equipment_name || !problem_description || !contact_person || !phone) {
        return res.status(400).json({ error: 'Не заполнены обязательные поля' });
    }
    ...
    db.run(query, [client_name, equipment_name, equipment_model || null, serial_number || null, problem_description, contact_person, phone, urgency || 'средняя'], function(err) {
        if (err) {
...
app.put('/api/service-requests/:id/assign', (req, res) => {
    ...
    if (!technician) {
        return res.status(400).json({ error: 'Не указан техник' });
    }
...
app.put('/api/service-requests/:id/solution', (req, res) => {
    ...
    if (!solution_description) {
        return res.status(400).json({ error: 'Не указано решение' });
    }
